/**
 * Checks ai-logo the way a consumer gets it: resolved through node_modules to the
...
describe("ai-logo, resolved as a dependency", () => {
  ...
  // Not every brand has every variant — OpenAI is monochrome and has no Color, for
  // one — so the toc's own flags decide what to expect. That makes this a check
  // that the metadata and the exports agree, rather than a guess about either.
  it.each(SAMPLE.map(({ Icon, name }) => [name, Icon] as const))(
    ...
    (name, Icon) => {
      ...
      // The types are per brand — OpenAI's genuinely has no Color — so each variant
      // is reached through an `in` narrowing rather than by indexing the union.
      expect({
  ...
  it("emits gradient ids under this fork's prefix", () => {
    // The de-branding renamed these from lobe-icons-* to ai-logo-*. Consumers see
    // the ids in their own DOM, so this is the end-to-end proof it reached them.
